  ...
  return (
    <MainContainer>
      <div className="relative rounded-[30px] bg-[#01092299] md:w-full max-w-[calc(100%_-_40px)] xl:max-w-[1200px] w-full mx-auto bg-white overflow-hidden">
        ...
        <div className="w-full mx-auto bg-white  flex lg:flex-row flex-col items-center lg:gap-[23px] justify-center pt-[72px] lg:pb-[75px] lg:pl-[51px] lg:pr-4 md:pt-[34px] pb-[40px] pl-4 pr-6 gap-[27px]">
          <div className="text-center md:text-left lg:text-left lg:max-w-[calc(62%_-_13px)] w-full max-w-full">
            <h1 className="text-lg md:2xl lg:text-3xl font-bold text-center lg:leading-9">
              {t("Description.title")}
            </h1>

            <div className="w-full py-[13px] pr-[9px] pl-[16px] rounded-[8px] border-[1px] border-[#E0E0E0] mt-1.5 md:mt-2.5">
              <p className="text-base md:text-base text-[#4F4F4F] lg:text-xl leading-[25.2px] text-left" dangerouslySetInnerHTML={{ __html: t("Description.description") }}>
              </p>
            </div>
            ...
            <div className="lg:mt-[29px] mt-[27px]">
              <h1 className="text-lg md:2xl lg:text-3xl font-bold text-center leading-9">{t("Description.Benefits")}</h1>
                ...
                {descriptionList.map((item, index) => {
                  if (index % 4 === 1 || index % 4 === 2) {
                    return (<div className={`flex flex-col gap-2.5 lg:gap-1 items-center max-w-full lg:max-w-[calc(50%_-_9px)] w-full lg:px-0 p-4 lg:pt-[19px] lg:pb-[12px] lg:bg-[#E7F5FA] rounded-3xl sm:max-w-full ${index % 2 === 0 ? 'bg-[#1D3E6A]' : 'bg-[#E7F5FA]'}`}>
                      <p
                        className={`text-xl lg:text-2xl lg:leading-7 lg:!text-black font-bold ${index % 2 === 0 ? 'text-white' : 'text-black'}`}
                        dangerouslySetInnerHTML={{
                          __html: t(`Description.${Object.keys(item)[0]}`),
                        }}
                      />
                      <p className={`text-center lg:min-h-[90px] md:h-auto px-1 flex justify-center items-center lg:text-black lg:text-lg text-base leading-5 ${index % 2 === 0 ? 'text-white' : 'text-black'}`}>{`${t(`Description.${Object.keys(item)[1]}`)}`}</p>
                    </div>
                    )
                  }
                  else {
                    return (<div className={`flex flex-col gap-2.5 lg:gap-1 items-center max-w-full lg:max-w-[calc(50%_-_9px)] w-full lg:px-0 p-4 lg:pt-[19px] lg:pb-[12px] lg:bg-[#1D3E6A] rounded-3xl  ${index % 2 === 0 ? 'bg-[#1D3E6A]' : 'bg-[#E7F5FA]'}`}>
                      <p
                        className={`text-xl lg:text-2xl lg:leading-7 lg:!text-white font-bold ${index % 2 === 0 ? 'text-white' : 'text-black'} `}
                        dangerouslySetInnerHTML={{
                          __html: t(`Description.${Object.keys(item)[0]}`),
                        }}
                      />
                      <p className={`text-center lg:min-h-[90px] px-1 flex justify-center items-center lg:!text-white lg:text-lg text-base leading-5 ${index % 2 === 0 ? 'text-white' : 'text-black'}`}>{`${t(`Description.${Object.keys(item)[1]}`)}`}</p>
                    </div>
                    )
                  }
          ...
          <div className="lg:ml-auto relative ml-0 lg:max-w-[calc(38%_-_13px)] w-full flex lg:flex-col items-center ">
